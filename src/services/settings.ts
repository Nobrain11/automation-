...
export function parseTpTiers(
  ...
  const tiers = parsed.map((tier) => {
    ...
    if (profit <= 0) {
      throw new Error(
        "TP profit must be greater than 0."
      );
    }
    ...
    if (
      ...
      throw new Error(
        "TP sell percentage must be 1–100."
      );
    }
  ...
  const total = tiers.reduce(
    (sum, tier) =>
      sum + tier.sellPercent,
    0
  );

  if (total > 100) {
    throw new Error(
      "TP sell percentages cannot exceed 100%."
    );
  }
...
export function serializeTpTiers(
  tiers: TakeProfitTier[]
): string {
  return JSON.stringify(
    parseTpTiers(JSON.stringify(tiers))
  );
}
