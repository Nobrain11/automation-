...
/*
 * Real referral stats pulled from the DB — referral code,
 * how many people the user has referred, and commission
 * actually earned (which will be 0 for everyone until the
 * trading engine exists and starts crediting real trades —
 * never fabricated).
 */
...
Referral info isn't available yet — open the menu again in a moment.
`.trim();
  }
...
Your code: <code>${info.code}</code>
Your link:
<code>${link}</code>

Referred users: <b>${info.referredCount}</b>
Commission rate: <b>${info.commissionRate}%</b> of their trades
Total earned: <b>${info.totalEarnedSol} SOL</b>

Commission is credited automatically once your referrals start trading. No trades have been recorded yet.
`.trim();
}
...
Tap a wallet to switch to it, or add a new one.
`.trim();
}
